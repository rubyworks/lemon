...
Fix reported issues (#6) to get lemon passing it own tests. (Note: the
test need to be run with test/fixtures in the loadpath). There was 
also a typo in the Outline reporter which was fixed. And better support
of the newst ANSI gem now lets color be deactivate witht he --no-ansi
command line option.
